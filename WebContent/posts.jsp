...
<body>
  ...
  <div class="topnav grid grid-cols-2 card shadow-lg  h-20 items-center">
     ...
     <div class="flex justify-end">
        <a href="posts.jsp" class="text-black-400 text-lg tracking-wide mr-4 cursor-pointer border-b-2 rounded-sm border-blue-600">Feed</a>
        ...
        <form action="logout">
         ...
         <sql:setDataSource var="db" driver="com.mysql.cj.jdbc.Driver"  
  		...
  		<sql:query dataSource="${db}" var="posts">  
			SELECT * from posts JOIN users ON users.email=posts.email WHERE posts.email IN (SELECT email FROM users JOIN relations AS r ON r.usermail="${email}" AND email=r.frndmail AND r.status=TRUE) ORDER BY post_time DESC;  
		</sql:query>
  
  <div class="posts flex-col mx-80 mt-10 w-1/2 ">
   		  <div class="post rounded p-5">
   		  <h1 class="text-lg tracking-wider text-black-500 font-bold">Feed</h1>
	        ...
	        <div class="rounded card shadow-md mb-4 p-5 mt-3">
	         ...
	         <h3 class="text-lg font-semi-bold text-black-500 tracking-wide">${j.uname}</h3>   
	        </div>
	        <hr class="text-gray-300 mt-2 font-semibold">
	        ...
	        <img src="${j.image}" class=" mt-2 w-full h-3/5 rounded "/> 
	         ...
	         <div class="flex">
	           ...
	           <input class="hidden" value="${j.postid}" name="id">
	          	<button class="rounded w-24 h-8 font-semi-bold tracking-wide bg-blue-400 text-gray-50 text-lg mr-2" type="submit">Like</button> 
	           </form>
	           <form action="dislike" method="POST">
	           <input class="hidden" value="${j.postid}" name="id">
	          	<button class="rounded w-24 h-8 font-semi-bold tracking-wide bg-red-400 text-gray-50 text-lg mr-0" type="submit">Dislike</button> 
	           </form>
	         </div>
